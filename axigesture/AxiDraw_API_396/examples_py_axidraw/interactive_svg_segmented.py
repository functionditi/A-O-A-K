...
def generate_recent_svg():
    ...
    # If the most recent update is an edge, compute its decoration as in full drawing.
    if recent_update and recent_update['type'] == 'edge':
        i = len(framework) - 1  # index of the new edge in framework
        edge = framework[i]
        dot1 = pullis[edge['x']]
        dot2 = pullis[edge['y']]
        # Draw basic line.
        ...
        # To mimic full framework, if this is the first edge use one style;
        # otherwise, compare with previous edge's angle.
        if i == 0:
            drawLineByAngle(angleDegrees, tempX, tempY, spacing, reverse=True)
            # Also draw an arc (as in the full code, a blue arc around the starting dot).
            ...
            if i % 2 == 1:
                ...
                if aDiff == 90 or aDiff == -270:
                    ...
                    drawLineByAngle(angleDegrees, tempX, tempY, spacing, reverse=True)
    # If the recent update is an arc (from branch termination), draw that arc.
    ...
    print("SVG (recent update only) saved as output.svg")

# --- Interactive Loop ---
def interactive_loop():
    resetPattern()
    generate_recent_svg()  # Save initial drawing.
    print("Interactive DFS SVG generator (recent update only, full decoration)")
    print("Commands:")
    ...
    while True:
        ...
        generate_recent_svg()  # Save an SVG containing only the new segment with full decoration
