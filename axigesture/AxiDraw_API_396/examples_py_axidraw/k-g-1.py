...
def generate_recent_svg():
    ...
    if recent_update and recent_update['type'] == 'edge':
        i = len(framework) - 1  # new edge index
        edge = framework[i]
        dot1 = pullis[edge['x']]
        dot2 = pullis[edge['y']]
...
def plot_svg_on_axidraw():
    ...
    try:
        ...
        # You can set additional options here, for example:
...
def interactive_loop():
    resetPattern()
    generate_recent_svg()  # Save initial SVG
    ...
    while True:
        ...
        if cmd == "1":
            ...
            pass  # just saving below
        elif cmd == "p":
            generate_recent_svg()  # Update SVG first
            plot_svg_on_axidraw()
            continue  # Skip saving again below
        elif cmd == "q":
            print("Exiting.")
            break
        else:
