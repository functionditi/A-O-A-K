...
# Using grid_size = 3 (3 grid points per row/column) and spacing = 50.
...
# Global variable to hold the current AxiDraw instance.
current_ad = None
...
# --- Function to Draw Base Grid ---
def draw_base_grid(ad):
    # For each dot, mark it with a tiny line (simulating a dot).
    dot_marker = 1  # 1 pixel length
    for dot in pullis:
        ad.moveto(to_inches(dot.x), to_inches(dot.y))
        ad.lineto(to_inches(dot.x + dot_marker), to_inches(dot.y))
