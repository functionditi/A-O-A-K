...
def serial_only_mode(ad):
    print("Entering serial-only mode. Press q and Enter to exit this mode.")
    try:
    ...
    while True:
        if ser.in_waiting:
            ...
            if line:
                print("Serial gesture:", line)
                if line == "1":
                    extendSameDirection()
                elif line == "2":
                    extendTurn()
                # Gesture "0" is ignored.
                send_recent_update_to_axidraw(ad)
        user_input = input("Press Enter to continue serial-only mode, or type q and Enter to exit: ").strip()
