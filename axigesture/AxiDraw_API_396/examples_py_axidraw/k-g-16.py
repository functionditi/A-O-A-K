...
# Grid settings: 3x3 grid, spacing = 50 pixels.
...
def terminateBranch_interactive(ad):
    ...
    if not dfsStack:
        return
    # Draw cap arc update and send it.
    currentIndex = dfsStack[-1]
    currentDot = pullis[currentIndex]
    r_angle = math.atan2(lastVector['dy'], lastVector['dx']) + math.pi if lastVector else 0
    cap_info = {'dot': currentDot, 'angle': r_angle, 'start': math.pi/4, 'stop': math.pi*7/4}
...
def interactive_draw_letter(letter, x, y, angle, ad):
    """
    Placeholder: draws a small rectangle and a horizontal line to represent the letter.
    In a full implementation, you'd convert 'letter' to vector outlines.
    """
    padding = 4
    letter_width = 10  # placeholder width in pixels
    letter_height = 12  # placeholder height in pixels

    half_w = letter_width / 2 + padding / 2
    half_h = letter_height / 2 + padding / 2

    # Compute rectangle corners (centered at 0,0).
    corners = [
        (-half_w, -half_h),
        ( half_w, -half_h),
        ( half_w,  half_h),
    ...
    ad.moveto(to_inches(pts[0][0]), to_inches(pts[0][1]))
    for (px, py) in pts[1:]:
        ad.lineto(to_inches(px), to_inches(py))
    # Draw a horizontal line as a placeholder for the letter.
    ad.moveto(to_inches(x - letter_width/2), to_inches(y))
    ad.lineto(to_inches(x + letter_width/2), to_inches(y))

def interactive_draw_text_on_line(x1, y1, x2, y2, txt, letter_count, ad):
    angle = math.atan2(y2 - y1, x2 - x1)
    for i, letter in enumerate(txt[:letter_count]):
        t = i / (letter_count - 1) if letter_count > 1 else 0.5
        x = x1 + t * (x2 - x1)
        y = y1 + t * (y2 - y1)
        interactive_draw_letter(letter, x, y, angle, ad)

def interactive_draw_text_on_arc(cx, cy, diameter, start, stop, txt, letter_count, ad):
    radius = diameter / 2
    angleDiff = stop - start
    for i, letter in enumerate(txt[:letter_count]):
        t = i / (letter_count - 1) if letter_count > 1 else 0.5
        angle = start + t * angleDiff
        x = cx + radius * math.cos(angle)
        y = cy + radius * math.sin(angle)
        letter_angle = angle + math.pi/2  # Tangent to arc.
        interactive_draw_letter(letter, x, y, letter_angle, ad)

# --- Draw Base Grid ---
def draw_base_grid(ad):
    marker_length = 1  # 1 pixel marker
    for dot in pullis:
...
def serial_only_mode(ad):
    ...
    try:
        ser = serial.Serial('/dev/tty.usbmodem1301', 9600, timeout=0.1)
    ...
    while True:
        ...
        if ser.in_waiting:
            ...
            if line:
                ...
                # Map Arduino output: "1" → extendSameDirection, "0" or "2" → extendTurn.
                if line == "1":
                    extendSameDirection()
...
def interactive_loop():
    ...
    draw_base_grid(ad)
    print("Interactive DFS AxiDraw generator with branch reversal, decorated reverse edges, and text drawing")
    print("Commands:")
    print("  1: Extend in same direction (Arduino '1' gives same)")
    print("  2: Extend with a turn (Arduino '0' or '2' gives turn)")
    print("  0: Terminate branch (reverse branch with decorations)")
    print("  t: Draw text along a line")
    print("  a: Draw text along an arc")
    print("  s: Enter serial-only mode")
    print("  p: Send recent update to AxiDraw")
    print("  q: Quit (press q twice to exit completely)")
    
    try:
        ser = serial.Serial('/dev/tty.usbmodem1301', 9600, timeout=0.5)
        print("Opened serial on /dev/tty.usbmodem1301 at 9600 baud.")
    except Exception as e:
        print("Could not open serial port:", e)
        ser = None
    ...
    while True:
        ...
        if cmd == "1":
            ...
            # For demonstration, draw text along a line between first two grid dots.
            if len(pullis) >= 2:
                x1, y1 = pullis[0].x, pullis[0].y
                x2, y2 = pullis[1].x, pullis[1].y
                interactive_draw_text_on_line(x1, y1, x2, y2, "KOLAM", 5, ad)
            continue
        elif cmd == "a":
            # For demonstration, draw text along an arc at the center of the grid.
            cx = (pullis[0].x + pullis[-1].x) / 2
            cy = (pullis[0].y + pullis[-1].y) / 2
            interactive_draw_text_on_arc(cx, cy, 100, math.pi/4, 3*math.pi/4, "KOLAM", 5, ad)
            continue
        elif cmd == "s":
            serial_only_mode(ad)
...
def interactive_draw_text_on_arc(cx, cy, diameter, start, stop, txt, letter_count, ad):
    ...
    for i, letter in enumerate(txt[:letter_count]):
        ...
        x = cx + radius * math.cos(angle)
        y = cy + radius * math.sin(angle)
        letter_angle = angle + math.pi/2
        interactive_draw_letter(letter, x, y, letter_angle, ad)

def interactive_draw_letter(letter, x, y, angle, ad):
    # Placeholder: draw a rectangle (background) and a horizontal line for the letter.
    padding = 4
    letter_width = 10
    letter_height = 12
    half_w = letter_width / 2 + padding / 2
    half_h = letter_height / 2 + padding / 2
    # Compute corners (centered at 0,0)
    corners = [
        (-half_w, -half_h),
        ( half_w, -half_h),
        ( half_w,  half_h),
    ...
    for (px, py) in pts[1:]:
        ad.lineto(to_inches(px), to_inches(py))
    # Draw a placeholder horizontal line representing the letter.
    ad.moveto(to_inches(x - letter_width/2), to_inches(y))
    ad.lineto(to_inches(x + letter_width/2), to_inches(y))
