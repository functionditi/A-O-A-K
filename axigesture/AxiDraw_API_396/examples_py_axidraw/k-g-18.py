...
def terminateBranch_interactive(ad):
    ...
    # Reverse the branch as in the original p5.js code.
...
# --- Auto-Generate Mode ---
def auto_mode(ad):
    print("Entering auto mode: generating random gesture every 10 seconds.")
...
def interactive_loop():
    ...
    print("  r: Enter auto mode (random gesture every 10 seconds)")
    ...
    while True:
        ...
        if cmd == "1":
            ...
            if confirm.lower() == "q":
                ...
                ad.penup()
                ad.moveto(0, 0)  # Home the machine: move pen to (0,0) and raise pen.
                print("Exiting.")
                break
            else:
                continue
        ...
        send_recent_update_to_axidraw(ad)
        # unvisitedIndices = [i for i in range(len(pullis)) if i not in visited]
        # if not unvisitedIndices:
        #     print("All dots visited. Homing AxiDraw and exiting...")
        #     current_ad.penup()
        #     current_ad.moveto(0, 0)
        #     break
...
def renderPermanentLayer(ad):
    # Render decorations (without drawing underlying framework).
    angleArray = []
