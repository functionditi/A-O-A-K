...
def drawDiagonalLine(midX, midY, lineLength, angle):
    ...
    # Add only the decoration (blue line).
...
def generate_recent_svg():
    ...
    # For plotting, we do not want any background shapes that cause unintended lines.
    # On the very first save, we display the grid (for reference) but with no stroke.
    if not grid_drawn:
        # Draw grid (black dots) without any background rectangle.
        for dot in pullis:
            current_svg.add(current_svg.circle(center=(dot.x, dot.y), r=2.5, fill='black', stroke='none'))
        grid_drawn = True
    else:
        # For subsequent updates, start with a plain white background with no stroke.
        current_svg.add(current_svg.rect(insert=(0, 0), size=(width, height), fill='white', stroke='none'))
    ...
    if dfsStack:
        ...
        current_svg.add(current_svg.circle(center=(currentDot.x, currentDot.y), r=5, fill='rgb(0,255,0)', stroke='none'))
...
def plot_svg_on_axidraw():
    ...
    try:
        ...
        # Prevent AxiDraw from homing between plots.
        ad.options.home_after = False
        # Additional plotting options can be set here.
        ad.plot_run()
        print("Plot sent to AxiDraw; it resumes from the last position.")
...
def interactive_loop():
    ...
    while True:
        ...
        if cmd == "1":
            ...
            plot_svg_on_axidraw()
            continue  # Skip further save below.
        elif cmd == "q":
            print("Exiting.")
            break
        else:
