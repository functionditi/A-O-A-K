...
# Conversion factor: pixels per inch in our drawing.
...
# Record most–recent update (edge or arc)
recent_update = None
...
def resetPattern():
    ...
    lastVector = None
    recent_update = None
    # Create grid of dots (offset by spacing)
...
# --- Interactive Drawing Functions for AxiDraw ---
# These functions convert our decoration routines into interactive moves.

def interactive_draw_diagonal_line(angleDegrees, tempX, tempY, spacing_val, reverse, ad):
    # Replicates drawLineByAngle from the p5.js version.
    ...
    # Compute endpoints as in drawDiagonalLine.
    x1 = tempX - math.cos(angle) * (spacing_val * 0.33)
    y1 = tempY - math.sin(angle) * (spacing_val * 0.33)
    x2 = tempX + math.cos(angle) * (spacing_val * 0.33)
    y2 = tempY + math.sin(angle) * (spacing_val * 0.33)
...
def interactive_loop_around(dot, theAngle, start, stop, ad, segments=30):
    # Replicates loopAround from p5.js.
    # In p5.js, we translate to (dot.x, dot.y), rotate by theAngle, then draw arc at (0,0) with width=spacing*0.66.
    # We'll approximate the arc.
    r = (spacing * 0.66) / 2
    pts = []
    ...
    # Move pen up to first point then draw.
...
def send_recent_update_to_axidraw(ad):
        ...
        if i == 0:
            ...
            if i % 2 == 1:
                if aDiff == 90 or aDiff == -270:
                    ...
                    # Draw loop decoration (using same parameters as applyLoopAndStroke)
    ...
    # Do not issue a home command; the AxiDraw will resume from its last position.

# --- Interactive Loop ---
def interactive_loop():
    resetPattern()
    # Initialize AxiDraw in interactive mode.
    ad = axidraw.AxiDraw()
    ad.interactive()
    if not ad.connect():
        print("AxiDraw not connected.")
        return
    # Set options so it does not home after plotting.
