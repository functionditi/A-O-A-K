...
# These functions issue interactive moves to the AxiDraw.
def interactive_draw_diagonal_line(angleDegrees, tempX, tempY, spacing_val, reverse, ad):
    # Mimic p5.js drawLineByAngle & drawDiagonalLine.
...
def interactive_loop_around(dot, theAngle, start, stop, ad, segments=30):
    # Mimic p5.js loopAround: translate to dot, rotate by theAngle, then draw arc from start to stop.
    r = (spacing * 0.66) / 2
    pts = []
...
def send_recent_update_to_axidraw(ad):
    ...
    if recent_update['type'] == 'edge':
        ...
        if i == 0:
            # For first edge: draw reversed diagonal and full arc.
            ...
            if i % 2 == 1:
                if aDiff == 90 or aDiff == -270:
                    ...
                    # Half-arc decoration for odd edges:
                    if aDiff == 0:
    ...
    # No homing command is issued; the AxiDraw resumes from its last position.
...
def interactive_loop():
    ...
    print("Interactive DFS AxiDraw generator (refined arcs and half-arcs)")
    print("Commands:")
