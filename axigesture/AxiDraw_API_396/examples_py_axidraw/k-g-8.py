...
def interactive_draw_diagonal_line(angleDegrees, tempX, tempY, spacing_val, reverse, ad):
    # Mimics p5.js drawLineByAngle & drawDiagonalLine.
...
def interactive_apply_loop_and_stroke(aDiff, r_angle, dot, ad):
    # Implements the original p5.js applyLoopAndStroke.
    if approx_equal(aDiff, 0):
        # Draw arc from PI/4 to 3*PI/4 using r_angle.
...
def send_recent_update_to_axidraw(ad):
    ...
    # No homing command; AxiDraw resumes from its last position.
...
def interactive_loop():
    ...
    print("Interactive DFS AxiDraw generator (refined half-arcs)")
    print("Commands:")
